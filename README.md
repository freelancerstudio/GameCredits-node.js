...
* Provide a one-stop resource for the Node.js developer to get started with Gamerscoin integration.
* Prevent would-be Gamerscoin web developers worrying whether a Gamerscoin client will work out of the box.
* Promote Node.js development of Gamerscoin web apps.
* Identify and address any incompatibilities with the Gamerscoin and Bitcoin APIs that exist now and/or in the future.

## Dependencies

You'll need a running instance of [gamerscoind](https://github.com/gamers-coin/gamers-coinv3) to connect with. If you're running Debian/Ubuntu, this worked for me: https://github.com/johngame/Gamerscoin-Auto-Daemon-Install-Script

Then, install the node-gamerscoin NPM package.

`npm install node-gamerscoin`
...
gamerscoin.auth('myusername', 'mypassword')

gamerscoin.getDifficulty(function() {
...
```js
gamerscoin.getNewAddress(function(err, address) {
...
```js
gamerscoin.exec('getNewAddress')

gamerscoin.exec('getbalance', function(err, balance) {
...
var gamerscoin = require('gamerscoin')({
    user:'user'
})

gamerscoin.set('pass', 'somn')
gamerscoin.set({port:40001})
...
You may pass an optional function `passphrasecallback` to the `node-gamerscoin` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `gamerscoind` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `gamerscoind` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `gamerscoin.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/gamerscoind.crt
    rpcsslprivatekeyfile=/etc/ssl/private/gamerscoind.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `gamerscoind.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-gamerscoin is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('gamerscoind.crt')
...
nodeunit test/test-node-gamerscoin.js
```
